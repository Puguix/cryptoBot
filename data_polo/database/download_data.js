...
const exchange_limit = JSON.parse(
    fs.readFileSync("./database/exchange_limit.json", "utf8")
);
const tf_ms = JSON.parse(fs.readFileSync("./database/tf_ms.json", "utf8"));
const coin_list = JSON.parse(
    fs.readFileSync("./database/coin_list.json", "utf8")
);
...
function timestamp_to_date(my_tf) {
    ...
    str_date = `${my_date.getUTCDate()}-${
        my_date.getUTCMonth() + 1
    }-${my_date.getUTCFullYear()} ${my_date.getUTCHours()}:${my_date.getUTCMinutes()}`;
    return str_date;
}
...
async function get_ohlcv(
    ...
    for (const tf in tf_array) {
        ...
        exchange
            ...
            .catch((err) => {
                console.log(
                    "Error retrieving candles since",
                    tf_array[tf],
                    exchange_name,
                    pair_name,
                    timeframe
    ...
    while (current_request < total_request) {
        process.stdout.write(
            `\rLoading ${current_request}/${total_request} requests | ${result_ohlcv.length} candles loaded`
        );
        await delay(2000);
    }
    process.stdout.write(
    ...
    csvWriter
        .writeRecords(result_ohlcv) // returns a promise
        .then(() => {
            process.stdout.write(
                `\rSuccessfully downloaded ${result_ohlcv.length} candles since ${first_date} in ${filepath}`
            );
            return true;
        })
        .catch((err) => {
...
async function get_multi_ohlcv(
    exchange,
    pair_list,
    tf_list,
    start_date,
